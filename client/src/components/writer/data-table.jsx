...
import { readNovelsByWriter, updateNovelStatus } from "@/api/writer"
...
const DataTable = () => {
  ...
  // โหลดนิยายของผู้เขียน
  const fetchNovels = async (pageNumber = 1) => {
    if (!user?.user_id) return
    try {
      setLoading(true)
      const res = await readNovelsByWriter(user.user_id, pageNumber, pageSize)
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Select
            value={novel.status}
            onValueChange={(value) => handleStatusChange(novel.novel_id, value)}
          >
  ...
  if (loading) return <div className="p-4">กำลังโหลดนิยาย...</div>
    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between text-sm text-gray-500 py-4">
        ...
        <div className="flex space-x-2">
          <Button variant="outline" size="sm" onClick={() => fetchNovels(page - 1)} disabled={page <= 1}>ก่อนหน้า</Button>
          <Button variant="outline" size="sm" onClick={() => fetchNovels(page + 1)} disabled={page >= totalPages}>ถัดไป</Button>
        </div>
      </div>
    </div>
  )
